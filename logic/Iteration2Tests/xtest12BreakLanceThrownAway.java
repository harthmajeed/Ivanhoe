...
public class xtest12BreakLanceThrownAway {
	...
	public void test() {
		...
		Card p2Card = new Card("Action", 0, false, false, false, false, false, false, false, false, true, false, false, false, false, false, false, false, false, false, false);
		p1Hand.add(p1Card);
		p1Hand.add(p1Card2);
		p2Hand.add(p2Card);
		...
		// player 2 has 2 cards in his hand
		assertEquals(2, game.getPlayerArray().get(1).getHand().size());
		//player 2 plays break lance
		game.update(1, 0);
		//player 1 loses all purple cards except the last card in his display.
		assertEquals(4, game.getPlayerArray().get(0).getPlayedValue());
		
		//player 2 now only has one card in his hand, break lance removed
		assertEquals(1, game.getPlayerArray().get(1).getHand().size());
		//there are no cards in player 2's display
		assertEquals(0, game.getPlayerArray().get(1).getDisplay().size());
		//player 2 has no face up cards.
