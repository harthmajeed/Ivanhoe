...
public class test4blue1 {
	...
	public void test() {
		...
		//add blue card to player 3's hand
		...
		//player 2 ends turn.
		game.update(1, true);
		//it is now player 3's turn, he draws a card thus his hand contains the 8 original cards plus the drawn card plus the added card, total = 10.
		assertEquals(true, game.getPlayerArray().get(2).isMyTurn());
		assertEquals (10, game.getPlayerArray().get(2).getHand().size());
		//player 3 plays one card(blue card)
		game.update(2, 8);
		//player 3 ends turn.
		game.update(2, true);
