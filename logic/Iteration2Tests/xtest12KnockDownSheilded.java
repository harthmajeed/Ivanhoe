...
public class xtest12KnockDownSheilded {
	...
	public void test() {
		...
		//set player 2 to shielded
		game.getPlayerArray().get(1).setSheilded(true);
		
		game.getPlayerArray().get(0).setTargetPlayer(1);
		//player 1 plays knock down
		game.update(0, 0);
	    //knock down has no effect, not removed from hand, not played
		assertEquals(2, game.getPlayerArray().get(0).getHand().size());
		assertEquals(2, game.getPlayerArray().get(1).getHand().size());
		assertEquals(0, game.getPlayerArray().get(0).getDisplay().size());
	}
